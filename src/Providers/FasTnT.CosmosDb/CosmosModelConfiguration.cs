...
namespace FasTnT.CosmosDb
{
    public class CosmosModelConfiguration : IModelConfiguration
    {
        public virtual void Apply(ModelBuilder modelBuilder)
        {
            ...
            storedQuery.Ignore(x => x.Subscriptions);
            storedQuery.HasData
            (
                new { Id = Guid.NewGuid(), Name = "SimpleEventQuery", DataSource = "SimpleEventQuery" },
                new { Id = Guid.NewGuid(), Name = "SimpleMasterDataQuery", DataSource = "SimpleMasterDataQuery" }
            );
        }
    }
}
