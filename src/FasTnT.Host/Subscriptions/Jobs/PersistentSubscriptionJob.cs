                ...
                if (scheduler.IsDue())
                {
                    ...
                    try
                    {
                        ...
                        var eventIds = pendingEvents
                            .Where(x => !_subscription.BufferRequestIds.Contains(x.RequestId))
                            .Select(x => x.Id);
                        var events = await context.Set<Event>()
                            .Where(x => eventIds.Contains(x.Id))
                            .ToListAsync(cancellationToken);

                        if (events.Any() || _subscription.ReportIfEmpty)
                        {
                            await SendResults(events, cancellationToken);
                        }

                        context.Attach(_subscription);

                        _subscription.BufferRequestIds = pendingEvents.Select(e => e.RequestId).Distinct().ToArray();
                    ...
                    {
                        await SendError(ex, cancellationToken);
                    }
                    finally
                    {
