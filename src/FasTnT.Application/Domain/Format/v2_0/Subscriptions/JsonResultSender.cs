...
namespace FasTnT.Application.Domain.Format.v2_0.Subscriptions;
...
public class JsonResultSender : IResultSender
{
    ...
    {
        var context = response.EventList.SelectMany(x => x.Fields).Select(x => x.Namespace).Where(x => !string.IsNullOrEmpty(x)).Distinct().BuildContext();
