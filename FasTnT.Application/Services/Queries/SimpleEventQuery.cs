...
namespace FasTnT.Application.Queries.Poll
{
    public class SimpleEventQuery : IEpcisQuery
    {
        private static readonly IDictionary<string, Func<IQueryable<Event>, QueryParameter, IQueryable<Event>>> SimpleParameters = new Dictionary<string, Func<IQueryable<Event>, QueryParameter, IQueryable<Event>>>
        {
            ...
            { "GE_eventTime",            (query, param) => query.Where(x => x.EventTime >= param.GetValue<DateTime>()) },
            { "LT_eventTime",            (query, param) => query.Where(x => x.EventTime < param.GetValue<DateTime>()) },
            { "GE_recordTime",           (query, param) => query.Where(x => x.Request.CaptureDate >= param.GetValue<DateTime>()) },
            { "LT_recordTime",           (query, param) => query.Where(x => x.Request.CaptureDate < param.GetValue<DateTime>()) },
            ...
            { "WD_readPoint",            (query, param) => throw new EpcisException(ExceptionType.ImplementationException, "WD_readPoint parameter is not implemented") },
            { "WD_bizLocation",          (query, param) => throw new EpcisException(ExceptionType.ImplementationException, "WD_bizLocation parameter is not implemented") },
