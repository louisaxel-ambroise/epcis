...
public sealed class SubscriptionBackgroundService : BackgroundService, ISubscriptionListener
{
    ...
    private readonly ConcurrentDictionary<CustomSubscription, DateTime> _scheduledExecutions = new();
    private readonly ConcurrentDictionary<string, List<CustomSubscription>> _triggeredSubscriptions = new();
    ...
    {
        if (subscription is CustomSubscription customSubscription)
        {
            Pulse(() =>
            {
                if (string.IsNullOrEmpty(subscription.Trigger))
                {
                    _scheduledExecutions[customSubscription] = customSubscription.Schedule.GetNextOccurence(DateTime.UtcNow);
                }
                else
                {
                    if (!_triggeredSubscriptions.ContainsKey(customSubscription.Trigger))
                    {
                        _triggeredSubscriptions[customSubscription.Trigger] = new ();
                    }

                    _triggeredSubscriptions[customSubscription.Trigger].Add(customSubscription);
                }
            });
        }
